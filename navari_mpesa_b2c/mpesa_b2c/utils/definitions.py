...
class B2CRequestDefinition:
    ...
    def to_dict(self, with_kwargs: dict[str, str] | None = None) -> dict:
        """Returns a Dictionary representation of the Dataclass, excluding fields Setting,
        ConsumerKey, and ConsumerSecret

        Args:
            with_kwargs (dict[str, str] | None): Additional dict like object to add to the
        ...
        if isinstance(with_kwargs, dict):
            filtered_dict.update(with_kwargs)

        return filtered_dict

    def to_json(self, with_kwargs: dict[str, str] | None = None) -> str:
        ...
        """
        return json.dumps(self.to_dict(with_kwargs))
